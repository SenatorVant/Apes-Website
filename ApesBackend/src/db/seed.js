...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS mfg_jobs (
    id          INTEGER PRIMARY KEY AUTOINCREMENT,
    job_id      TEXT UNIQUE,           -- e.g. MFG-001
    part_number TEXT,
  ...
  CREATE TABLE IF NOT EXISTS events (
    id          INTEGER PRIMARY KEY AUTOINCREMENT,
    title       TEXT NOT NULL,
    date_str    TEXT NOT NULL,       -- stored as "Apr 26-27" or ISO date
  ...
  CREATE TABLE IF NOT EXISTS attendance (
    ...
    present     INTEGER DEFAULT 1,  -- 1 = present, 0 = absent
    notes       TEXT,
  ...
  -- File attachments (for tasks, meetings, mfg jobs)
  CREATE TABLE IF NOT EXISTS attachments (
    ...
    mime_type   TEXT,
    size_bytes  INTEGER,
    entity_type TEXT,               -- 'task' | 'meeting' | 'mfg'
    entity_id   INTEGER,
...
const seedUsers = db.transaction(() => {
  for (const u of ROSTER) {
    const id = uuidv4();
    userIds[u.email] = id;
    const password_hash = bcrypt.hashSync(PASSES[u.role], 10);
    insertUser.run({ id, ...u, password_hash });
  }
});
seedUsers();
// reload real ids from db (in case rows already existed)
for (const u of ROSTER) {
...
const insertProj = db.prepare(`
  INSERT OR IGNORE INTO projects (name, description, subteam, status, progress, due_date)
  VALUES (@name, @description, @subteam, @status, @progress, @due_date)
`);
const PROJECTS = [
...
const insertTask = db.prepare(`
  INSERT OR IGNORE INTO tasks (title, description, assigned_to, project_id, category, status, progress, due_date)
  VALUES (@title, @description, @assigned_to, @project_id, @category, @status, @progress, @due_date)
`);
const projRow = (name) => db.prepare('SELECT id FROM projects WHERE name=?').get(name);
const TASKS = [
...
for (const t of TASKS) {
  const p = projRow(t.project);
  insertTask.run({ ...t, project_id: p ? p.id : null });
}

// Manufacturing jobs
const insertMfg = db.prepare(`
  INSERT OR IGNORE INTO mfg_jobs (job_id, part_number, description, machine, material, revision, status, assigned_to, drawing_ref)
  VALUES (@job_id, @part_number, @description, @machine, @material, @revision, @status, @assigned_to, @drawing_ref)
`);
const MFG = [
...
for (const m of MFG) insertMfg.run(m);

// Purchase orders
const insertPO = db.prepare(`
  INSERT OR IGNORE INTO purchase_orders (po_id, part_number, part_name, price, quantity, vendor_link, requested_by, justification, project_id, status, notes)
  VALUES (@po_id, @part_number, @part_name, @price, @quantity, @vendor_link, @requested_by, @justification, @project_id, @status, @notes)
`);
const swerveId = projRow('Swerve Drive Module')?.id;
const ORDERS = [
...
for (const o of ORDERS) insertPO.run(o);

// Events
const insertEv = db.prepare(`
  INSERT OR IGNORE INTO events (title, date_str, type, visibility, link, description)
  VALUES (@title, @date_str, @type, @visibility, @link, @description)
`);
const EVENTS = [
...
// Meetings
const insertMtg = db.prepare(`
  INSERT OR IGNORE INTO meetings (day, month, title, summary, tags)
  VALUES (@day, @month, @title, @summary, @tags)
`);
const MEETINGS = [
...
for (const m of MEETINGS) insertMtg.run(m);

// Attendance (10 sessions for each seeded user)
...
const insertAtt = db.prepare(`
  INSERT OR IGNORE INTO attendance (user_id, session_date, present)
  VALUES (@user_id, @session_date, @present)
`);
for (const [email, arr] of Object.entries(attData)) {
  const uid = userIds[email];
  if (!uid) continue;
  arr.forEach((p, i) => insertAtt.run({ user_id: uid, session_date: sessions[i], present: p }));
}
